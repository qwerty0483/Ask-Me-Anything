...
if (isset($_POST['boardIdx'], $_POST['userId'], $_POST['boardLike'])) {
    ...
    $likeSql = "SELECT * FROM information_like WHERE boardIdx = '$boardIdx' AND userId = '$userId';";
    $likeResult = mysqli_query($db_conn, $likeSql);

    if (mysqli_num_rows($likeResult) > 0) {
        $updateSql = "UPDATE information_like SET boardLike = '$boardLike' WHERE boardIdx = '$boardIdx' AND userId = '$userId';";
        mysqli_query($db_conn, $updateSql);
    } else {
        $insertSql = "INSERT INTO information_like (boardIdx, userId, boardLike) VALUES ('$boardIdx', '$userId', '$boardLike');";
        mysqli_query($db_conn, $insertSql);
    }

    $countChange = $boardLike == 1 ? 1 : -1;
    $updateCountSql = "UPDATE information_list SET boardGood = boardGood + $countChange WHERE boardIdx = '$boardIdx';";
