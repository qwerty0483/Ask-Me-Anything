...
if (isset($_POST['boardIdx'], $_POST['userId'], $_POST['boardScrap'])) {
    ...
    $scrapSql = "SELECT * FROM information_scrap WHERE boardIdx = '$boardIdx' AND userId = '$userId';";
    $scrapResult = mysqli_query($db_conn, $scrapSql);

    if (mysqli_num_rows($scrapResult) > 0) {
        $updateSql = "UPDATE information_scrap SET boardscrap = '$boardScrap' WHERE boardIdx = '$boardIdx' AND userId = '$userId';";
        mysqli_query($db_conn, $updateSql);
    } else {
        $insertSql = "INSERT INTO information_scrap (boardIdx, userId, boardScrap) VALUES ('$boardIdx', '$userId', '$boardScrap');";
        mysqli_query($db_conn, $insertSql);
    }

    $countChange = $boardScrap == 1 ? 1 : 0;
    $updateCountSql = "UPDATE information_scrap SET boardScrap = $countChange WHERE boardIdx = '$boardIdx';";
